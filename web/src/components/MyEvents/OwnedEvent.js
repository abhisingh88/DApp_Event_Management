...
import {
  TextField, ExpansionPanel, ExpansionPanelSummary, Button, Select, FormHelperText,
...
export default class OwnedEvent extends Component {
            ...
            <FormControl>
              ...
              <List dense={true}>
                {this.state.customer_list.map((e, i) =>
                  <ListItem key={i}>
                    {e} : <p>{this.state.ticket_Count[i]}</p>
                  </ListItem>
                )}
              </List>
  ...
  view_funds = async () => {
    if (!this.state.view_funds) {
      try {
        this.setState({
          funds: await this.props.contract.methods.view_funds(
            this.props.eventId
  ...
  get_customer_list = async () => {
    try {
      this.setState({
        customer_list: await this.props.contract.methods.get_customers(
          this.props.eventId
        ).call({ from: this.props.accounts[0] })
      });

      this.state.customer_list.map(async (e) => {
        this.state.ticket_Count.push(
          await this.get_ticket_count(e)
        );
        // this.setState({ticket_Count:[...this.state.ticket_Count, await this.get_ticket_count(e)]})
      })
      console.log(this.state.ticket_Count);
    } catch (error) {
      console.log("Dev error: " + error.message);
    }
  }

  get_ticket_count = async (customer_Id) => {
    try {
      let data = await this.props.contract.methods.get_tickets(
        this.props.eventId, customer_Id
      ).call({ from: this.props.accounts[0] })
